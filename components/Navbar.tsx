...
const Navbar: React.FC<NavbarProps> = ({ onOpenDialog }) => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? "glass-nav py-4 border-b border-zinc-100 shadow-sm" : "py-8"}`}
    >
      ...
      <div
        className={`fixed inset-0 bg-white z-40 flex flex-col items-center justify-center transition-transform duration-500 ${mobileMenuOpen ? "translate-y-0" : "-translate-y-full"} md:hidden`}
      >
